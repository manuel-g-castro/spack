...
class Cp2k(MakefilePackage, CudaPackage):
    ...
    variant('smm', default='blas', values=('libxsmm', 'libsmm', 'blas'),
            description='Library for small matrix multiplications')
    variant('plumed', default=False, description='Enable PLUMED support')
    variant('libint', default=False,
    ...
    patch('cp2k-7.1.diff', when='@7.1')
    patch('cp2k-8.1.diff', when='@8.1')
    patch('cp2k-8.2.diff', when='@8.2')
    patch('cp2k-9.1.diff', when='@9.1')
...
#        elif '^fujitsu-fftw' in spec:
            ...
            fftw = spec['fujitsu-fftw:openmp' if '+openmp' in spec else 'fujitsu-fftw']
            fftw_header_dir = fftw.headers.directories[0]
...
#            'fj': ['-O1'],
        ...
        if '%intel' in spec:
            ...
            ldflags += ['-Kparallel -SSL2 -SCALAPACK']
            fcflags += ['-Free -X08 -Koptions']
#            if '@9.1' in spec:
#                fcflags += ['-Free -X08 -Koptions']
        elif '%gcc' in spec:
            fcflags += [
...
#            libs.extend(self.compiler.stdcxx_libs)
        ...
        with open(self.makefile, 'w') as mkf:
            ...
            if '%fj' in spec:
                mkf.write(fflags('LDFLAGS_C', ldflags + ['-mlcmain=main']))
