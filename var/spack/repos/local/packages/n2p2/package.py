...
class N2p2(MakefilePackage):
    ...
    def edit(self, spec, prefix):
        ...
        gsl_include = self.spec["gsl"].prefix.include
        eigen_include = self.spec["eigen"].prefix.include.eigen3
        cflags = self.compiler.cxx11_flag
        blas_libs = self.spec["blas"].libs

        makefile.filter(
            "PROJECT_GSL=.*", "PROJECT_GSL={0}".format(gsl_include)
        )

        makefile.filter(
            "PROJECT_EIGEN=.*", "PROJECT_EIGEN={0}".format(eigen_include)
        )
        ...
        makefile.filter(
            "PROJECT_CFLAGS=.*", "PROJECT_CFLAGS={0}".format(cflags)
        )
        makefile.filter(
