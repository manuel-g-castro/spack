      subroutine cmdbsl(e,l,rj,dj,rn,dn)
c-----------------------------------------------------------------------
c         Complex iversion of modified Bessel & Neuman functions
c         and their derivatives. e is complex number and integer
c         l=1,2,3...
c
c         j(z,l) = z**l * rj(l+1)
...
c         the Neuman function is differenti from the conventional one.
c         coded by H.Akai, 1975, Osaka
c         revised by H.Akai, 1989, Osaka
c-----------------------------------------------------------------------
      implicit real*8 (a-h,o-z)
      parameter (lmx=10, lmx2=lmx+2, lmx3=2*lmx+3)
      complex*16 rj(*),dj(*),rn(*),dn(*),f(lmx3),g(lmx3),e,a,b
      real*8 cj(lmx2)
      logical first
      save first,cj
   ...
   10 cj(j)=cj(j-1)/dble(2*j-1)
      ...
      write(6,1000)
 1000 format ('   ***err in cmdbsl...illegal l')
      stop
      endif
      do 30 i=1,2
      j=l+2-i
