...
c     among atoms in the unit cell. The meaning of the table is, e.g.,
c     if lineary aligned five atoms 1,2,3,4,5 change their position
c     such that they look like 4,5,1,2,3 after the operation, the vector
c     'irotat' has values (4,5,1,2,3). If the operation iop is
c     allowed, isymop(iop) is set to 1, otherwise 0.
c
c     Note: Since the procedure performed here is a bit complicated, the
c     style of the program seems not to be very ellegant, namely a lot of
...
c-----------------------------------------------------------------------
     ...
     &      ,gpt(3,ngpt),ftype(natm),dsplmt(3,48),gsf(ngpt)
      integer itype(natm),irotat(natm,48),isymop(48)
      logical eqvlat,str,dsp
      data zero/1d-8/,dsp/.true./
c
...
c     --- sometimes translation without rotation by a constant vector
c         transfomrs the crsytal to itself. This happens when supercell
c         is used. Also the case is when a rhombohedral lattice is
c         treated as a hexagonal lattice. Such cases have to be taken
c         care of.
...
c         symmetry is compatible with the symmetry operation "iop".
      isymop(iop)=0
c     --- Rotate the cartesian coordinates of all the atoms.
      call rotatm(atmicp,wk,protat(1,iop),natm)
c     --- Check if the rotated crystal generates the same
...
c         The first atom of this pair can always be the rotated atom-1.
      do 60 id=1,natm
      if(id .eq. 1 .and. iop .eq. 1) go to 60
...
c     --- A displacement is assumed.
   70 dsplmt(i,iop)=wk(i,1)-atmicp(i,id)
c     --- Now a displacement compatible to (id,1) pair has been set.   
c     write(*,'(1x,a,i2,a,3f10.5)')'target=',id,'   dsp=',dsplmt(:,iop)
      do 80 ib=2,natm
...
c     --- Check the shift for the (ia, ib) pair.
  ...
  100 c(j)=0d0
      do 110 i=1,3
      d=wk(i,ib)-atmicp(i,ia)-dsplmt(i,iop)
...
c     call errtrp(3,'atmrot','eqvlat detected no structure differences')
   ...
   10 continue
c     --- set isymop(1) because it might be set 0 if no translational
c         symmtery other than those of primitive unit cell vectors
c         exists.
      if(isymop(1) .eq. 0) then
      do 140 i=1,natm
  140 irotat(i,1)=i
      isymop(1)=1
      call clrarr(dsplmt(1,1),3)
      endif
c
c     --- optional output
      if(dsp) then 
...
c     write(*,'(i3,3f12.7)')(iop,dsplmt(:,iop),iop=1,48)
c     do 150 iop=1,48
c 150 write(*,'(i2,(t6,20i3))')iop,irotat(:,iop)
      end
