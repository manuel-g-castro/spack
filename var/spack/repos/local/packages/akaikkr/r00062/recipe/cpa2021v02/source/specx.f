...
c     parameter(ngmx=61, msr=400 ,nbuf=1000)
...
c     data ef0/0d0/, dex0/5d-2/, emxr/1.2d0/, msex/201/,mse0/35/
...
c     --- one-shot calculation with Fermi level adjustment will be done
      ids=9
      maxitr=100
      else if(ifkey('vc',go)) then
c     --- give the information of possible vacancy sites without any
c         real calculation.
      outtyp='quit'
      maxitr=1
      ids=10
...
c
      call sbtime(0,0)
c     latmx=6
c     ainit1=redata(atmicv(13))
c     ainit2=redata(atmicv(16))
c     aincr=1d-2
c     inirec=record
c     do 80 lat=1,latmx
c     a=ainit+dble(lat-1)*aincr
c     record=inirec
c     do 81 i=1,3
c     write(atmicv(i+12),*)ainit1-aincr*dble(lat-1)
c  81 write(atmicv(i+15),*)ainit2+aincr*dble(lat-1)
   80 call spmain
