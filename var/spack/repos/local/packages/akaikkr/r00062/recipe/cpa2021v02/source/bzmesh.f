      subroutine bzmesh(nf,nk,g,r,protat,isymop,ls,fbz)
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      ...
      real*8 g(3,3),r(3,3),vkp(3,nkx),wtkp(nkx),protat(9,24,2)
     &      ,t(3),s(3),bc(3,3),bctyp(3)
      integer isymop(24,2),nc(0:ibit-1)
      integer*4,allocatable::iwk(:)
      integer*1,allocatable::iwt(:)
      logical fbz,ls
      save iwt,nc,dw,nfa,nfb,nfc,da,db,dc,nkmx,ktot,istart,iwk
...
c         somewhat similar to the conventional k-point meshes.
      ...
      j=mod(l,ibit)
      if(mod(iwk(iadr)/nc(j),2) .eq. 0) then
c     --- Since it was not marked, it has not yet been assigned.
c     --- Mark this point.
      iwk(iadr)=iwk(iadr)+nc(j)
      a=da*dble(ia)
      t(1)=xb+a*g(1,1)
...
c     vkp(3,n)=zb+a*g(3,1)+zero
      wtkp(n)=dw*dble(iwt(ktot))
      if(n .eq. nkx .or. ktot .eq. nkmx) go to 160
      endif
  140 continue
      call errtrp(1,'bzmesh','nkmx not consistent')
...
c         length becomes as small as possible.
  ...
  100 vkp(i,k)=t(i)
   ...
   80 continue
      if(ktot .eq. nkmx) deallocate(iwt,iwk)
      end
