...
c-----------------------------------------------------------------------
      ...
      real*8 v(3),y(*),a(lb),b(lp,lp),t(lp,lp),pog(lp),c(lp),s(lp)
      logical init
      save b,pog,init
      data init/.true./, zero/1d-30/
c
...
c
      ...
      write(6,'(a,i4,a)')'   ***err in realh...lmax=',lmax
     &      ,' is illegal'
      stop
      endif
c
   ...
   60 t(l,1)=(dble(2*l-3)*v(3)*t(l-1,1)-r*dble(l-2)*t(l-2,1))/dble(l-1)
      ...
      do 70 n=2,l-1
      nn=l+1-n
   70 t(l,nn)=(r*dble(l+nn-2)*t(l-1,nn)-x3*t(l,nn+1))/dble(l-nn)/v(3)
      else
      do 80 l=3,la
      do 80 n=2,l-1
