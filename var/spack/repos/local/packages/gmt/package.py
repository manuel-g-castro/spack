...
class Gmt(Package):
    ...
    def install(self, spec, prefix):
        with working_dir('spack-build', create=True):
            ...
            if '+lapack' in spec:
                args.extend([
                    '-DLAPACK_INCLUDE_DIR={0}'.format(
                        spec['lapack'].headers.directories[0]),
                    '-DLAPACK_LIBRARY={0}'.format(
                        spec['lapack'].libs.joined(";")),
                    '-DGMT_EXCLUDE_LAPACK=TRUE'
                ])
            else:
                args.extend(['-DLAPACK_INCLUDE_DIR=', '-DLAPACK_LIBRARY='])

            if '+blas' in spec:
                args.extend([
                    '-DBLAS_INCLUDE_DIR={0}'.format(
                        spec['blas'].headers.directories[0]),
                    '-DBLAS_LIBRARY={0}'.format(
                        spec['blas'].libs.joined(";")),
                    '-DGMT_EXCLUDE_BLAS=TRUE'
                ])
            else:
                args.extend(['-DBLAS_INCLUDE_DIR=', '-DBLAS_LIBRARY='])
