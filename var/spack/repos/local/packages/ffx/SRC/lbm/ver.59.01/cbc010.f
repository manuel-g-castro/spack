      SUBROUTINE CBC010(ITIME,NG,NP,ILVL1,ILVL2,IPOSI,F,FEQ,
     ...
     *          IMODE,IOFFST,JOFFST,KOFFST,N1,IP,JP
      REAL*8    BUF(NP),COEF,FBUF,FBUF2,C1,C2,C3
C
C     ====FUNCTION====
C     SET B.C. DATA TO EDGE-TYPE B.C. WITH B.C. FLAG-TYPE OF 010
C
C     ====VARIABLES LIST====
C[IN]
C     ITIME         :TIME STEP
      ...
      ELSE IF(ILVL2.EQ. 1) THEN
          ...
          DO 2100 IP=1,NP
              ...
              DO 2200 JP=1,NP
                  ...
                  FBUF=FBUF+FBUF2
 2200         CONTINUE
              FBC2(IP,(I+1)/2) = FEQ(I,J,K,IP)+COEF*FBUF
 2100     CONTINUE
 2000     CONTINUE
      ELSE IF(ILVL2.EQ.-1) THEN
          N1=2**(ILVL1-1)
...
C
          ...
          K2=K1-1
          K3=K2-1
          C1= 3.0D0/8.0D0/2.0D0 
          C2= 3.0D0/4.0D0/2.0D0 
          C3=-1.0D0/8.0D0/2.0D0 
          ...
          DO 3100 I=1,NG+1
              ...
              DO 3400 IP=1,NP
                  FBUF
     *            = 0.5E0*(FEQ(I1,J1,K1,IP)+FEQ(I2,J1,K1,IP))
                  FBUF2
     *            = 0.5E0*(F(I1,J1,K1,IP)+F(I2,J1,K1,IP))
                  BUF(IP)=FBUF+COEF*(FBUF2-FBUF) 
 3400         CONTINUE   
              IF(IMODE.EQ.1) THEN
                  FBC2 (1:NP,I) = BUF(1:NP)
                  FBC2W(1:NP,I) = BUF(1:NP)
              ELSE IF(IMODE.EQ.2) THEN
                  FBC2 (1:NP,I) = 0.5D0*(BUF+FBC2W(1:NP,I))
              ENDIF
 3100     CONTINUE
      ENDIF
C
