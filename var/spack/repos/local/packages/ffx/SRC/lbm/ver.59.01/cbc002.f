      SUBROUTINE CBC002(ITIME,NG,NP,ILVL1,ILVL2,IPOSI,F,FEQ,
...
C
C     ====FUNCTION====
C     SET B.C. DATA TO FACE-TYPE B.C. WITH B.C. FLAG-TYPE OF 002
C
C     ====VARIABLES LIST====
C[IN]
C     ITIME         :TIME STEP
...
C
C[OUT]
C     FBC1 (IP,I,J) :WORK REGION FOR FACE-TYPE  B.C. GROUPS
C     FBC1W(IP,I,J) :WORK REGION FOR FACE-TYPE  B.C. GROUPS, TIME INTERPOLATION 
C
C
      IF(ILVL2.EQ.0) THEN
          I=NG
      ...
      ELSE IF(ILVL2.EQ. 1) THEN
          ...
          DO 2200 IP=1,NP
              ...
              DO 2300 JP=1,NP
                  ...
                  FBUF=FBUF+FBUF2
 2300         CONTINUE
              FBC1(IP,(J+1)/2,(K+1)/2) = FEQ(I,J,K,IP)+COEF*FBUF
 2200     CONTINUE
 2100     CONTINUE
 2000     CONTINUE
      ELSE IF(ILVL2.EQ.-1) THEN
...
C
          ...
          DO 3300 J=1,NG+1
              ...
              DO 3400 IP=1,NP
                  FBUF
     *            = ( FEQ(I1,J1,K1,IP)+FEQ(I1,J2,K1,IP)
     *               +FEQ(I1,J1,K2,IP)+FEQ(I1,J2,K2,IP))/4.0E0
                  FBUF2
     *            = ( F(I1,J1,K1,IP)+F(I1,J2,K1,IP)
     *               +F(I1,J1,K2,IP)+F(I1,J2,K2,IP))/4.0E0
                  BUF(IP)=FBUF+COEF*(FBUF2-FBUF) 
 3400         CONTINUE   
              IF(IMODE.EQ.1) THEN
                  FBC1 (1:NP,J,K) = BUF(1:NP)
                  FBC1W(1:NP,J,K) = BUF(1:NP)
              ELSE IF(IMODE.EQ.2) THEN
                  FBC1 (1:NP,J,K) = 0.5D0*(BUF+FBC1W(1:NP,J,K))
              ENDIF
 3300     CONTINUE
 3200     CONTINUE
      ENDIF
