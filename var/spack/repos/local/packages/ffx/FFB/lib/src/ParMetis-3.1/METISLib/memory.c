...
void AllocateWorkSpace(CtrlType *ctrl, GraphType *graph, int nparts)
{
  ...
  if (ctrl->optype == OP_KMETIS) {
    ctrl->wspace.edegrees = (EDegreeType *)GKmalloc(sizeof(EDegreeType)*(size_t)graph->nedges, "AllocateWorkSpace: edegrees");
    ...
    */
    ...
    ctrl->wspace.vedegrees = (VEDegreeType *)GKmalloc(sizeof(VEDegreeType)*(size_t)graph->nedges, "AllocateWorkSpace: vedegrees");
