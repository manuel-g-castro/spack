...
// modified by yoshimura 2013/06/10
//void rcapSetCADFilename( const char* filename )
void rcapSetCADFilename( const char* filein, int* ierr )
{
	...
	// added by yoshimura 2013/06/10 Begin
	char filename[61] = " ";
	int i=0;
	while( *(filein+i) != ' ' && i<61){*(filename+i)=*(filein+i);i++;}
	// added by yoshimura 2013/06/10 End
	kmb::RnfShapeIO rnfshape;
	//rnfshape.loadFromFile( filename, rcapRefinerDoc.surfaces );
	if (rnfshape.loadFromFile( filename, rcapRefinerDoc.surfaces ) == -1) {
	  *ierr = 1;
	  return;
	}
	// modified by yoshimura 2013/06/10 End

	kmb::RevocapNeutralIO rnf;
	// modified by yoshimura 2013/06/10 Begin
	//rnf.loadFromRNFFile( filename, rcapRefinerDoc.mesh );
	if (rnf.loadFromRNFFile( filename, rcapRefinerDoc.mesh ) == -1 ) {
	  *ierr = 1;
	  return;
	}
	// modified by yoshimura 2013/06/10 End
...
// modified by yoshimura 2013/06/19
//void rcapWriteFittingFile( const char* filename )
void rcapWriteFittingFile( const char* filein )
{
	if( rcapRefinerDoc.refiner == NULL ){
	...
	// added by yoshimura 2013/06/19 Begin
	char filename[61] = " ";
	int i=0;
	while( *(filein+i) != ' ' && i<61){*(filename+i)=*(filein+i);i++;}
	// added by yoshimura 2013/06/19 End
...
void rcapsetcadfilename_( const char* filename, int* ierr ){ rcapSetCADFilename( filename, ierr ); }
void rcapwritefittingfile_( const char* filename ){ rcapWriteFittingFile( filename ); }
// modified by yoshimura 2013/06/19 End
