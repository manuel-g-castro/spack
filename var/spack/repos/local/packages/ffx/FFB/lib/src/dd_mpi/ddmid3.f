...
C REAL*4   VGAT1  (IB) ; GATHERED VALUES AT MID NODES 
C REAL*4   VGAT2  (IB) ; GATHERED VALUES AT MID NODES 
C REAL*4   VGAT3  (IB) ; GATHERED VALUES AT MID NODES 
...
!ocl norecurrence(BUFSND)
          DO 2100 IPT = 1 , NPTGAT(IDOM)
              BUFSND(NSTART+4*(IPT-1)  ) = IPMID(IPT,IDOM)
              BUFSND(NSTART+4*(IPT-1)+1) = VGAT1(IPGAT(IPT,IDOM))
              BUFSND(NSTART+4*(IPT-1)+2) = VGAT2(IPGAT(IPT,IDOM))
              BUFSND(NSTART+4*(IPT-1)+3) = VGAT3(IPGAT(IPT,IDOM))
 2100     CONTINUE
C
#ifdef PRECEXP
...
!ocl norecurrence(BUFSND)          
          DO 2300 IPT = 1 , NPTSCT(IDOM)
              BUFSND(NSTART+7*(IPT-1)  ) = IEMID(IPT,IDOM)
              BUFSND(NSTART+7*(IPT-1)+1) = CSCT(1,IPSCT(IPT,IDOM))
              BUFSND(NSTART+7*(IPT-1)+2) = CSCT(2,IPSCT(IPT,IDOM))
              BUFSND(NSTART+7*(IPT-1)+3) = CSCT(3,IPSCT(IPT,IDOM))
              BUFSND(NSTART+7*(IPT-1)+4) = VSCT1( IPSCT(IPT,IDOM))
              BUFSND(NSTART+7*(IPT-1)+5) = VSCT2( IPSCT(IPT,IDOM))
              BUFSND(NSTART+7*(IPT-1)+6) = VSCT3( IPSCT(IPT,IDOM))
 2300     CONTINUE
C
#ifdef PRECEXP
          CALL MPI_ISEND(BUFSND(NSTART),MSGLEN,MPI_REAL8,ISEND,MSGTYP,
...
C
      ...
      DO 4100 IDOM = 1 , NGATR        
         DO 4200 IPT = 1 , NPTSCT(IDOM)
            IPT2=NSTART+IPT-1
            ISTART=4*(NSTART+IPT-2)+1
            LPGAT(IPT2)=BUFRCV(ISTART  )+0.1
            VGAT1(IPT2)=BUFRCV(ISTART+1)
            VGAT2(IPT2)=BUFRCV(ISTART+2)
            VGAT3(IPT2)=BUFRCV(ISTART+3)
 4200    CONTINUE
