...
C                                                                      C
C======================================================================C
      SUBROUTINE SRFEX3(MESURF,MLST,MB,NE,NP,N2,NEX,NS,NSP,N2D,
     *                  LPNT,NPNT,LPNT2,NPNT2,LOCAL,NODE,
     *                  LESURF,NESURF,NPNTN,MWRK,IWRK,IWRK2,IUT0,IERR)
C
      IMPLICIT NONE
C
      INTEGER*4 MESURF,MLST,MB,NE,NP,N2,NEX,NS,NSP,N2D,
     *          LPNT,NPNT,LPNT2,NPNT2,LOCAL,NODE,
     *          LESURF,NESURF,NPNTN,MWRK,IWRK,IWRK2,IUT0,IERR
C
      DIMENSION LPNT(MB),LPNT2(NPNT2),
     1          LOCAL(NSP,NS,4),NODE(N2,NE),NEX(8),
     2          LESURF(MLST,MESURF),IWRK(MWRK),IWRK2(MWRK)
C
      CHARACTER*60 ERMSGB
     ...
     & /' NUMBER OF SURFACE ELEMENTS EXCEEDED LIMIT OF               '/
      CHARACTER*60 EREXP4
     & /' NUMBER OF SYMMETRIC BOUNDARY NODE EXCEEDED LIMIT OF        '/
C
      INTEGER*4 IPNT,IP,IS,IE,IETYPE,NSURF,IBMAX,I,II,IPF,JP,J,J1,J2,
     *          IS1,IE1,IS2,IE2,IETYPE1,IETYPE2,LIST1,LIST2,NBUF,IMATCH
C
...
C
      DO 150 IPNT = 1 , NPNT2
          IF(LPNT2(IPNT).LT.1 .OR. LPNT2(IPNT).GT.NP) THEN
              WRITE(IUT0,*) ERMSGB
              WRITE(IUT0,*) EREXP2
              WRITE(IUT0,*) 
     &        ' *** POINT NO. = ',IPNT,' *** NODE NO. = ',LPNT2(IPNT)
              IERR = 1
          ENDIF
 150  CONTINUE
      IF(IERR.EQ.1) RETURN
         ...
         DO 401 IS=1,NSURF
            ...
            DO 402 I=1,N2D
               ...
               IF(II.GE.1) THEN
                  ...
                  IBMAX=MAX(IBMAX,IWRK(IPF))
               ENDIF
 402        CONTINUE
            IF (IBMAX.EQ.1) GOTO 401
            NESURF = NESURF +1
            LESURF(1,NESURF) = IE
            LESURF(2,NESURF) = IS
C
