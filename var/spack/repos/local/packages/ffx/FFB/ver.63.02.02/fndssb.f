      SUBROUTINE FNDSSB(N2,NE,NEG,NP,NEX,X,Y,Z,NODE,LIST,
     *                  XMINE,YMINE,ZMINE,XMAXE,YMAXE,ZMAXE,
     *                  XP,YP,ZP,EN,GP,EP,TP,IUT0,IERR)
      IMPLICIT NONE
C
***** DEFINE ARGUMENTS *****
      INTEGER IP,N2,NE,NEG,NP,NEX,NODE,LIST
      REAL X,Y,Z
      ...
      INTEGER EN
      REAL GP,EP,TP
      REAL*4 DX1,DY1,DZ1,RR
C
      DIMENSION NODE(N2,NEG),LIST(NE),NEX(8),X(NP),Y(NP),Z(NP),
...
C
          IE=LIST(IE0)
C
          IF(     (XP.LT.XMINE(IE)-EPS)
     *       .OR. (XP.GT.XMAXE(IE)+EPS)
     *       .OR. (YP.LT.YMINE(IE)-EPS)
     *       .OR. (YP.GT.YMAXE(IE)+EPS)
     *       .OR. (ZP.LT.ZMINE(IE)-EPS)
     *       .OR. (ZP.GT.ZMAXE(IE)+EPS) )
     *    GOTO 1000
C
          DO 1050 II=1,8
             IF (NODE(II,IE).EQ.0) GOTO 1050
             DX1=XP-X(NODE(II,IE))
             DY1=YP-Y(NODE(II,IE))
             DZ1=ZP-Z(NODE(II,IE))
             RR=DX1*DX1+DY1*DY1+DZ1*DZ1
             IF (RR.LE.EPS*EPS) GOTO 1000
 1050     CONTINUE
C
