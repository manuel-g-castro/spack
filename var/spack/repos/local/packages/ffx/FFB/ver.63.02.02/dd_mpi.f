...
C
     ...
     & / ' RECEIVED NODE NUMBER IS OUT OF THE GLOBAL NODE NUMBER     ' /
       ...
       INTEGER*4 ISAVE,LSTORE(MDATA),NDUP,LDUP(2,MDATA)
       DATA ISAVE /0/
       DATA NDUP  /0/
       SAVE ISAVE,LSTORE,NDUP,LDUP
C
C
C      EXCHANGE X, Y, AND Z RESIDUALS AMONG THE NEIGHBORING SUB-DOMAINS
C     AND SUPERIMPOSE THE EXCHANGED RESIDUALS TO THE CALLING TASK'S
...
C
CCTTDEBG
      IF(ISAVE.EQ.0 .AND. IDIM.EQ.1) THEN
          DO IP = 1, NP
              LSTORE(IP)=0
          ENDDO
C
...
C
          ...
          DO IDOM = 1 , NDOM
              DO IBP = 1, NBPDOM(IDOM)
                  IP = IPSND(IBP,IDOM)
                  NSTART2   = NSTART + NSKIP * (IBP-1)
                  IF(LSTORE(IP).GE.2) THEN
                      NDUP=NDUP+1
                      LDUP(1,NDUP)=IP
                      LDUP(2,NDUP)=NSTART2+1
                  ENDIF
              ENDDO
              NSTART = NSTART + NSKIP * NBPDOM(IDOM)
          ENDDO
...
CCTT SUBROUTINE DDCMAX END
      SUBROUTINE DDCOMY(IPART,LDOM,NBPDOM,NDOM,IPSLF,IPSND,MBPDOM,
     *                  FXYZ,NP,IUT0,IERR,BUFSND,BUFRCV,MAXBUF)
      IMPLICIT REAL*4(A-H,O-Z)
      DIMENSION LDOM(NDOM),NBPDOM(NDOM),IPSLF(MBPDOM,NDOM),
     1          IPSND(MBPDOM,NDOM), FXYZ(3,NP),
     2          BUFSND(MAXBUF),BUFRCV(MAXBUF)
C
      INCLUDE 'mpif.h'
C
...
C
      CHARACTER*60 ERMSGB
     & / ' ## SUBROUTINE DDCOMY: FATAL     ERROR OCCURRENCE; RETURNED' /
      CHARACTER*60 EREXP1
     & / ' DIMENSION SIZE OF INTERNAL      ARRAYS IS NOT SUFFICIENT  ' /
      CHARACTER*60 EREXP2
     & / ' DIMENSION SIZE OF PASSED BUFFER ARRAYS IS NOT SUFFICIENT  ' /
...
C
C       (3) INPUT-OUTPUT
C REAL*4   FXYZ(3,IP)       ; RESIDUAL VECTOR
C
C       (4) WORK
C REAL*4   BUFSND(IBUF)     ; HOLDS THE VALUES OF THE INTER-CONNECT
C                            BOUNDARY NODE NUMBER IN THE NEIGHBORING
...
!ocl norecurrence(BUFSND)
         DO IBP=1,NBPDOM(IDOM)
            NSTART2 = NSTART + NSKIP * (IBP-1)
            IP      = IPSLF(IBP,IDOM)
            BUFSND(NSTART2+1) = FXYZ(1,IP)
            BUFSND(NSTART2+2) = FXYZ(2,IP)
            BUFSND(NSTART2+3) = FXYZ(3,IP)
         ENDDO
 210  CONTINUE
...
!ocl norecurrence(FXYZ)
         DO IBP = 1, NBPDOM(IDOM)
            IP = IPSND(IBP,IDOM)
            NSTART2 = NSTART + NSKIP * (IBP-1)
            IF(LSTORE(IP).EQ.1) THEN
                FXYZ(1,IP) = FXYZ(1,IP) + BUFRCV(NSTART2+1)
                FXYZ(2,IP) = FXYZ(2,IP) + BUFRCV(NSTART2+2)
                FXYZ(3,IP) = FXYZ(3,IP) + BUFRCV(NSTART2+3)
            ENDIF
         ENDDO
         NSTART = NSTART + NSKIP * NBPDOM(IDOM)
      ENDDO
C
      DO I=1,NDUP
          IP=LDUP(1,I)
          J =LDUP(2,I)
          FXYZ(1,IP) = FXYZ(1,IP) + BUFRCV(J  )
          FXYZ(2,IP) = FXYZ(2,IP) + BUFRCV(J+1)
          FXYZ(3,IP) = FXYZ(3,IP) + BUFRCV(J+2)
      ENDDO 
C
      IF(IERR .eq. 1) THEN
        WRITE(IUT0,*) ERMSGB
