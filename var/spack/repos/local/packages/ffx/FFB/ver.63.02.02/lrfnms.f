      SUBROUTINE LRFNMS(IRFNFF,IRFN,NGRID,IRFNFT,FILECD,FILECR,
     *                  NLAY1,NLAY2,IRFBOX,NPPRS,
     *                  ME,MP,MWRK,N2,NE,NP,
     *                  X,Y,Z,U,V,W,PN,P,NODE,LEACNV,
     *                  MPWALL,NPWALL,LPWALL,UWALL,VWALL,WWALL,
     *                  MPINLT,NPINLT,LPINLT,UINLT,VINLT,WINLT,
     *                  MPFREE,NPFREE,LPFREE,
     *                  MPSYMT,NPSYMT,LPSYMT,
     *                  MPSET ,NPSET ,LPSET1,LPSET4,
     ...
     *                  CRD,LWRK01,LWRK02,LWRK03,
     *                  WRK04,NDRFN,NDORG,NODEBK,
     *                  RX,RY,NPB0, 
...
C
      INTEGER*4 IRFNFF,IRFN,NGRID,IRFNFT,NLAY1,NLAY2,IRFBOX,NPPRS
      INTEGER*4 ME,MP,MWRK,N2,NE,NP
...
C
     ...
     *          MPSET ,NPSET ,LPSET1(MPSET ),LPSET4(MPSET ),
...
C
      INTEGER*4 MPTGT,NPTGT,LPTGT(MPTGT)
      REAL*4    XRFMIN,YRFMIN,ZRFMIN,XRFMAX,YRFMAX,ZRFMAX
C
      CHARACTER*60 FILECD,FILECR
C
...
C
      ...
      DATA      LBPRTY( 8) /  8 /   ! REFINE TARGET
      CHARACTER*20 BGNAME(4)
      DATA         BGNAME( 1) / "GROUP1" /
      DATA         BGNAME( 2) / "GROUP2" /
      DATA         BGNAME( 3) / "GROUP3" /
      DATA         BGNAME( 4) / "GROUP4" /
      CHARACTER*10 CDOM
      CHARACTER*50 CBUF
C
C     [WORK]
      REAL*4    UFIX(MP),VFIX(MP),WFIX(MP)
      INTEGER*4 LPFRM(MWRK)
      REAL*8    CRD(MP*3)
      REAL*4    RX(ME*8),RY(ME*8)
      INTEGER*4 NODED(8),LEACNV(ME)
      INTEGER*4 LWRK01(MWRK),LWRK02(MWRK),LWRK03(MWRK)
      INTEGER*4 WRK04(MP),NDRFN(ME*8),NDORG(ME*8),NODEBK(8,ME)
...
CC   & /' ## SUBROUTINE RFNMSH: CANNOT OBTAIN PARENT NODE ; RETURNED' /
     ...
     & /' ## SUBROUTINE LRFNMS: INVARIT O.S. FRANE NUMBER ; RETURNED' /
      CHARACTER*60 ERMSG7
     & /' ## SUBROUTINE LRFNMS: CAN NOT READ CAD FILE     ; RETURNED' /
C
C
C     ARGUMENT LISTINGS
...
C     
CC
CCHY [*] ASSIGN ELEMENT PRESSURE TO EACH NODE
CC
      IF (NPPRS.EQ.0) THEN
C
         DO 100 IP=1,NP
...
C
         DO 200 IE=1,NE
            DO 210 I=1,8
               IP=NODE(I,IE)
               IF(IP.EQ.0) GOTO 210
               WRK04(IP)=WRK04(IP)+1.0E0
               UFIX (IP)=UFIX (IP)+P(IE)
 210        CONTINUE
 200     CONTINUE
C
         IDIM = 2
         CALL DDCOMX(IPART,IDIM,LDOM,NBPDOM,NDOM,IPSLF,IPSND,MBPDOM,
     *               WRK04,UFIX,WRK04,NP,IUT0,IERR,RX,RY,MAXBUF)
C
         DO 220 IP=1,NP
            PN(IP)=UFIX(IP)/WRK04(IP)
 220     CONTINUE
C
      ENDIF
C
...
CC
      ...
      CALL EXTRFN(NGRID,NLAY1,NLAY2,IRFBOX,
     *            ME,N2,NE,NP,NODE,X,Y,Z,
     *            NPBODY,LPBODY,NPINLT,LPINLT,NPTGT,LPTGT,
     *            XRFMIN,YRFMIN,ZRFMIN,XRFMAX,YRFMAX,ZRFMAX,
     *            LWRK01,LWRK02,LWRK03,LEACNV,
...
CC
      ...
      IF (IRFNFT.EQ.1) THEN
         WRITE(IUT6,*) ' **LRFNMS** : SHAPE FITTING : ON'
         CALL RCAPSETCADFILENAME(FILECD,IERR)
         CALL ERRCHK(IUT6,IPART,1,IERR,IERRA)
         IF(IERRA.NE.0) THEN
            WRITE(IUT0,*)
            WRITE(IUT0,*) ERMSG7
...
CC
 ...
 3300    CONTINUE
C
         CBUF = BGNAME(4)//CHAR(0)
         CALL RCAPAPPENDBNODEVARINT(CBUF,NPTGT,LWRK01,LWRK02)
         WRITE(IUT6,*) ' **LRFNMS** : SET B.C. NODES (4)', NPTGT
      ENDIF
CC
...
CC    COUNT NUMBER OF ARRAY
      ...
      IF(NE2+NER.GT.ME) THEN
          WRITE(IUT0,*) ERMSG3
          WRITE(IUT0,*) "NE2+NER / ME", NE2+NER, ME
          IERR=1
      ENDIF
      CALL ERRCHK(IUT6,IPART,1,IERR,IERRA)
      IF(IERRA.NE.0) THEN
...
C
     ...
     *WRITE(IUT6,*) ' **LRFNMS** : NUMBER OF FREE        NODES:',NPFREE
      IF(NPSET.GT.0)
     *WRITE(IUT6,*) ' **LRFNMS** : NUMBER OF OVERSET     NODES:',NPSET
CC
...
C
         DO 6160 IB=1,NB
            IP=LWRK01(IB)
            IF (IP.LE.NPOLD) GOTO 6160
            NPTGT=NPTGT+1
            IF (NPTGT.GT.MPTGT) THEN
               WRITE(IUT0,*) ERMSG3
               IERR=1
               GOTO 6170
            ENDIF
...
C
         IF(NB.GT.0) THEN
            WRITE(IUT6,*)
     *      ' **LRFNMS** : NUMBER OF REFINE TARGET        NODES:',NPTGT
         ENDIF
C
      ENDIF
CC
...
CC
      IF (IRFNFT.EQ.1) THEN
         FILECD=FILECR
         CALL RCAPWRITEFITTINGFILE(FILECD)
      ENDIF
C
